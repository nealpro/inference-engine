...
test "GGUF parser reads metadata and tensor directory" {
    ...
    try std.testing.expectEqual(model.QuantizationType.f32, parsed.tensors[0].kind);
...
fn fixtureGguf(allocator: std.mem.Allocator, unsupported_quant: bool) ![]u8 {
    ...
    try appendInt(&bytes, allocator, u64, 8);
    ...
    try appendInt(&bytes, allocator, u64, 2);
    const tensor_kind: u32 = if (unsupported_quant) 999 else 0;
    ...
    try appendF32(&bytes, allocator, 1.0);
    try appendF32(&bytes, allocator, 0.0);
    try appendF32(&bytes, allocator, 0.0);
    try appendF32(&bytes, allocator, 1.0);
...
fn appendF32(bytes: *std.ArrayList(u8), allocator: std.mem.Allocator, value: f32) !void {
    try appendInt(bytes, allocator, u32, @as(u32, @bitCast(value)));
}
